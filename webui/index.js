...
window.newChat = async function () {
  try {
    // Wait a bit and retry if Alpine.js isn't ready
    if (!window.Alpine || !chatsSection || !chatsSection._x_dataStack) {
      console.warn('Alpine or chatsSection not available for newChat, retrying...');
      ...
      setTimeout(() => {
        if (window.Alpine && chatsSection && chatsSection._x_dataStack) {
          ...
          window.toast("Please wait for the interface to fully load", "info");
        }
      }, 100);
      return;
    }
...
window.killChat = async function (id) {
  ...
  if (!window.Alpine || !chatsSection || !chatsSection._x_dataStack) {
    ...
    window.toast("Please wait for the interface to fully load", "error");
    return;
  }
...
window.selectChat = async function (id) {
  ...
  if (!tabSwitched) {
    ...
    if (!window.Alpine || !chatsSection || !chatsSection._x_dataStack) {
      console.warn('Alpine or chatsSection not available for selectChat');
      return;
    }
...
export const setContext = function (id) {
  ...
  // Update both selected states only if Alpine is ready
  if (window.Alpine && chatsSection && chatsSection._x_dataStack) {
    const chatsAD = Alpine.$data(chatsSection);
    const tasksSection = document.getElementById("tasks-section");
...
function scrollChanged(isAtBottom) {
  if (window.Alpine && autoScrollSwitch && autoScrollSwitch._x_dataStack) {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.addEventListener(
    ...
    (e) => {
      dragCounter++;
      if (dragCounter === 1 && window.Alpine && dragDropOverlay && dragDropOverlay._x_dataStack) {
  ...
  document.addEventListener(
    ...
    (e) => {
      dragCounter--;
      if (dragCounter === 0 && window.Alpine && dragDropOverlay && dragDropOverlay._x_dataStack) {
      ...
      if (modalEl._x_dataStack) {
        ...
        setTimeout(() => {
          ...
          setTimeout(() => {
            ...
            if (schedulerComponent._x_dataStack) {
