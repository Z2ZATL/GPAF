...
The following user guide provides instructions for installing and running GPAF using Docker, which is the primary runtime environment for the framework. For developers and contributors, we also provide instructions for setting up the [full development environment](#in-depth-guide-for-full-binaries-installation).

### Need updates from v0.7? 👉[How to update GPAF](#how-to-update-gpaf)
...
- Docker Desktop provides the runtime environment for GPAF, ensuring consistent behavior and security across platforms
...
1.5. Create a Docker Hub account when prompted and sign in. This is required to pull the GPAF container image.
...
2.1. Pull the GPAF Docker image:
- Search for `frdel/gpaf-run` in Docker Desktop
...
- Choose or create a directory on your machine where you want to store GPAF's data
- This can be any location you prefer (e.g., `C:/gpaf-data` or `/home/user/gpaf-data`)
- This directory will contain all your GPAF files, like the legacy root folder structure:
  - `/memory` - GPAF's memory and learned information
  - `/knowledge` - Knowledge base
  - `/instruments` - Instruments and functions
  - `/prompts` - Prompt files
  - `/work_dir` - Working directory
  - `.env` - Your API keys
  - `settings.json` - Your GPAF settings

> [!TIP]
> Choose a location that's easy to access and backup. All your GPAF data 
> will be directly accessible in this directory.

2.3. Run the container:
- In Docker Desktop, go back to the "Images" tab
- Click the `Run` button next to the `frdel/gpaf-run` image
- Open the "Optional settings" menu
- Set the port to `0` in the second "Host port" field (for automatic port assignment)
- Under "Volumes", configure:
  - Host path: Your chosen directory (e.g., `C:\gpaf-data`)
  - Container path: `/a0`
...
> Alternatively, run the following command in your terminal:
> ```bash
> docker run -p $PORT:80 -v /path/to/your/data:/a0 frdel/gpaf-run
> ```
...
- The Web UI will open. GPAF is ready for configuration!
...
> [!NOTE]
> After starting the container, you'll find all GPAF files in your chosen 
...
3. Configure GPAF
- Refer to the following sections for a full guide on how to configure GPAF.

## Settings Configuration
GPAF provides a comprehensive settings interface to customize various aspects of its functionality. Access the settings by clicking the "Settings"button with a gear icon in the sidebar.

### GPAF Configuration
- **Prompts Subdirectory:** Choose the subdirectory within `/prompts` for GPAF behavior customization. The 'default' directory contains the standard prompts.
- **Memory Subdirectory:** Select the subdirectory for GPAF memory storage, allowing separation between different instances.
- **Knowledge Subdirectory:** Specify the location of custom knowledge files to enhance the GPAF's understanding.
...
Learn more about Remote Function Calls and their purpose [here](#7-configure-gpaf-rfc).

> [!IMPORTANT]
> Always keep your API keys and passwords secure.

# Choosing Your LLMs
The Settings page is the control center for selecting the Large Language Models (LLMs) that power GPAF.  You can choose different LLMs for different roles:
...
> mixed in the vector database. Note that this will DELETE ALL of GPAF's memory.
...
#### Selecting your model within GPAF
...
## Using GPAF on your mobile device
GPAF's Web UI is accessible from any device on your network through the Docker container:
...
> [!NOTE]
> If you're running GPAF directly on your system (legacy approach) instead of 
> using Docker, you'll need to configure the host manually in `run_ui.py` to run on all interfaces using `host="0.0.0.0"`.

For developers or users who need to run GPAF directly on their system,see the [In-Depth Guide for Full Binaries Installation](#in-depth-guide-for-full-binaries-installation).

# How to update GPAF

1. **If you come from the previous version of GPAF (pre-0.7.1):**
- Your data is safely stored across various directories and files inside the GPAF folder.
- To update to the new Docker runtime version, you need to save the following files and directories:
  - `/memory` - GPAF's memory
  ...
  - `/work_dir` - Working directory
  - `.env` - Your API keys
  - `/tmp/settings.json` - Your GPAF settings
...
- Reach for the folder where you saved your data and copy it to the new GPAF folder set during the installation process.
- GPAF will automatically detect your saved data and use it across memory, knowledge, instruments, prompts and settings.

> [!IMPORTANT]
> Make sure to use the same embedding model you were using before, otherwise 
> you will have to re-index all of GPAF's memory, therefore deleting all 
> your custom knowledge and memory.
>
> If you have issues loading your settings, you can try to delete the `/tmp/settings.json` 
> file and let GPAF generate a new one.
...
- Go to "Images" tab and remove the `frdel/gpaf-run` image or click the three dots to pull the difference and update the Docker image.
...
> your GPAF data directory (the chosen directory on your machine).
...
3. **Full Binaries**
- Using Git/GitHub: Pull the latest version of the GPAF repository. 
...
- GPAF is a framework. It's made to be customized, edited, enhanced. Therefore you need to install the necessary components to run it when downloading its full binaries. This guide will help you to do so.
- The following step by step instructions can be followed along with a video for this tutorial on how to make GPAF work with its full development environment.
...
2. You don't necessarily need API keys: GPAF can run with local models. For this tutorial though, we will leave it to the default OpenAI API. A guide for downloading Ollama along with local models is available [here](#installing-and-using-ollama-local-models).
...
6. Running GPAF without Docker makes the process more complicated and it's thought for developers and contributors.
...
## 2. Download GPAF
- You can clone the GPAF repository (https://github.com/frdel/gpaf) from GitHub if you know how to use Git. In this tutorial I will just show how to download the files.

1. Go to the GPAF releases [here](https://github.com/frdel/gpaf/releases).
2. The latest release is on the top of the list, click the "Source Code (zip)" button under "Assets" to download it.

<img src="res/setup/image-14-u.png" alt="gpaf download" width="500"/>
<br><br>

3. Extract the downloaded archive where you want to have it. I will extract them to "gpaf" folder on my Desktop - "C:\Users\frdel\Desktop\gpaf" on Windows and "/Users/frdel/Desktop/gpaf" on macOS.
...
2. In the terminal, navigate to your GPAF folder using **"cd"** command. Replace the path with your actual GPAF folder path.
~~~
cd C:\Users\frdel\Desktop\gpaf
~~~
You should see your folder has changed on the next terminal line.

<img src="res/setup/image-15.png" alt="gpaf cd" height="100"/>
<img src="res/setup/image-16.png" alt="gpaf cd" height="100"/>
<br><br>

3. Create Conda environment using command **"conda create"**. After **"-n"** is your environment name, you can choose your own, i will use **"a0"** - short for GPAF. After **"python"** is the Python version that Conda will install for you into this environment, right now, 3.12 works fine. **-y** skips confirmations.
~~~
...
Simply put, Docker is a way of running virtual computers on your machine. These are lightweight, disposable and isolated from your operating system, so it is a way to sandbox GPAF.
- GPAF only connects to the Docker container when it needs to execute code and commands. The frameworks itself runs on your machine.
...
> Docker socket to be used (requires password)."  This allows GPAF to 
...
- GPAF needs a Docker image to be pulled from the Docker Hub to be run, even when using the full binaries.
You can refer to the [installation instructions above](#windows-macos-and-linux-setup-guide) to run the Docker container and then resume from the next step. There are two differences:
  ...
  - You need to map the `/a0` volume to the location of your local GPAF folder.
- Run the Docker container following the instructions.

## 5. Run the local GPAF instance
Run the GPAF with Web UI:
~~~
python run_ui.py
~~~
...
- Open the URL shown in terminal in your web browser. You should see the GPAF interface.

## 6. Configure GPAF
Now we can configure GPAF - select models, settings, API Keys etc. Refer to the [Usage](usage.md#agent-configuration) guide for a full guide on how to configure GPAF.

## 7. Configure GPAF RFC
GPAF needs to be configured further to redirect some functions to the Docker container. This is crucial for development as A0 needs to run in a standardized environment to support all features.
...
9. Now you have the full development environment to work on GPAF.
...
### Conclusion
After following the instructions for your specific operating system, you should have GPAF successfully installed and running. You can now start exploring the framework's capabilities and experimenting with creating your own intelligent agents. 

If you encounter any issues during the installation process, please consult the [Troubleshooting section](troubleshooting.md) of this documentation or refer to the GPAF [Skool](https://www.skool.com/gpaf) or [Discord](https://discord.gg/Z2tun2N3) community for assistance.
