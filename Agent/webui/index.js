...
export async function sendMessage() {
    try {
        ...
        // Check if inputSection and Alpine are available
        if (!window.Alpine || !inputSection || !inputSection._x_dataStack) {
            console.warn('Alpine or inputSection not available for sendMessage');
            return;
        }
...
function setConnectionStatus(connected) {
    ...
    try {
        ...
        if (statusIconElement && statusIconElement._x_dataStack) {
    ...
    try {
        ...
        if (window.Alpine && inputSection && inputSection._x_dataStack) {
            try {
                ...
                console.warn('Error updating input paused state:', error);
            }
        }
        ...
        if (window.Alpine && chatsSection && chatsSection._x_dataStack) {
            try {
                ...
                console.warn('Error updating chats contexts:', error);
            }
        }

        // Update tasks list and sort by creation time (newer first)
        const tasksSection = document.getElementById('tasks-section');
        if (tasksSection && window.Alpine && tasksSection._x_dataStack) {
            try {
                ...
                console.warn('Error updating tasks list:', error);
            }
        }
        ...
        if (context) {
            ...
            if (activeTab === 'chats' && window.Alpine && chatsSection && chatsSection._x_dataStack) {
                try {
                    const chatsAD = Alpine.$data(chatsSection);
                    ...
                    console.warn('Error updating chats selection:', error);
                }
            } else if (activeTab === 'tasks' && tasksSection && window.Alpine && tasksSection._x_dataStack) {
                try {
                    const tasksAD = Alpine.$data(tasksSection);
                    ...
                    console.warn('Error updating tasks selection:', error);
                }
            }
            ...
            if (tasksSection && window.Alpine && tasksSection._x_dataStack) {
                try {
                    ...
                    console.warn('Error setting first task selection:', error);
                }
            }
        } else if (response.contexts?.length > 0 && localStorage.getItem('activeTab') === 'chats') {
            // If we're in chats tab with no selection but have chats, select the first one
            const firstChatId = response.contexts[0].id;
            ...
            if (!context) {
                ...
                if (window.Alpine && chatsSection && chatsSection._x_dataStack) {
                    try {
                        ...
                        console.warn('Error setting first chat selection:', error);
                    }
                }
            }
        }
...
window.newChat = async function () {
    ...
    try {
        ...
        updateAfterScroll()
        toast("New chat created", "success", 2000);
    } catch (e) {
...
window.settings = async function () {
    ...
    try {
        // Call the settings modal proxy to open the settings
        if (window.settingsModalProxy) {
            await window.settingsModalProxy.openModal();
            console.log("Settings modal opened successfully");
        } else {
            console.error("settingsModalProxy not found");
            toast("Settings modal not available", "error");
        }
    } catch (e) {
        console.error("Error in settings:", e);
        window.toastFetchError("Error opening settings", e)
    }
}
...
window.killChat = async function (id) {
    ...
    try {
        if (!window.Alpine || !chatsSection || !chatsSection._x_dataStack) {
            console.warn('Alpine or chatsSection not available for killChat');
            // Fallback: just switch context and delete on server
        ...
        try {
            ...
            console.warn('Error accessing Alpine data in killChat:', alpineError);
            // Fallback: still delete the chat from server
            switchFromContext(id);
            await sendJsonData("/chat_remove", { context: id });
            toast("Chat deleted successfully", "success");
        }
...
export function switchFromContext(id) {
    ...
    if (context === id) {
        if (!window.Alpine || !chatsSection || !chatsSection._x_dataStack) {
            console.warn('Alpine or chatsSection not available for switchFromContext');
        ...
        try {
            ...
            console.warn('Error in switchFromContext:', error);
            // Fallback: create a new context
            setContext(generateGUID());
        }
    }
...
function ensureProperTabSelection(contextId) {
    ...
    if (tasksSection && window.Alpine && tasksSection._x_dataStack) {
        try {
            ...
            console.warn('Error accessing tasks data in ensureProperTabSelection:', error);
        }
    }
    ...
    if (!tabSwitched) {
        ...
        if (window.Alpine && chatsSection && chatsSection._x_dataStack) {
            try {
                ...
                console.warn('Error updating chats selection in selectChat:', error);
            }
        }
        ...
        if (tasksSection && window.Alpine && tasksSection._x_dataStack) {
            try {
                ...
                console.warn('Error updating tasks selection in selectChat:', error);
            }
        }
...
export const setContext = function (id) {
    ...
    // Update both selected states with error handling
    if (window.Alpine) {
        try {
            if (chatsSection && chatsSection._x_dataStack) {
                const chatsAD = Alpine.$data(chatsSection);
                chatsAD.selected = id;
            }
        } catch (error) {
        ...
        try {
            if (tasksSection && tasksSection._x_dataStack) {
                const tasksAD = Alpine.$data(tasksSection);
                tasksAD.selected = id;
            }
        } catch (error) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Add fallback event listeners for buttons in case Alpine.js fails
    setTimeout(() => {
...
function setupButtonFallbacks() {
    console.log("Setting up button fallback event listeners");
    
    // Reset Chat button
    const resetBtn = document.getElementById('resetChat');
    if (resetBtn && !resetBtn.hasAttribute('data-fallback-attached')) {
        resetBtn.addEventListener('click', function(e) {
            console.log("Reset button clicked (fallback)");
            if (window.resetChat) {
                window.resetChat();
            }
        });
        resetBtn.setAttribute('data-fallback-attached', 'true');
    }
    
    // New Chat button
    const newChatBtn = document.getElementById('newChat');
    if (newChatBtn && !newChatBtn.hasAttribute('data-fallback-attached')) {
        newChatBtn.addEventListener('click', function(e) {
            console.log("New chat button clicked (fallback)");
            if (window.newChat) {
                window.newChat();
            }
        });
        newChatBtn.setAttribute('data-fallback-attached', 'true');
    }
    
    // Settings button
    const settingsBtn = document.getElementById('settings');
    if (settingsBtn && !settingsBtn.hasAttribute('data-fallback-attached')) {
        ...
        settingsBtn.setAttribute('data-fallback-attached', 'true');
    }
}
...
function scrollChanged(isAtBottom) {
    // Check if autoScrollSwitch element exists before trying to access Alpine data
    if (autoScrollSwitch && autoScrollSwitch._x_dataStack) {
        try {
            const inputAS = Alpine.$data(autoScrollSwitch);
            inputAS.autoScroll = isAtBottom;
        } catch (error) {
            console.warn('Error accessing Alpine data for autoScrollSwitch:', error);
        }
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.addEventListener('dragenter', (e) => {
        dragCounter++;
        if (dragCounter === 1) {
            if (window.Alpine && dragDropOverlay && dragDropOverlay._x_dataStack) {
                try {
                    Alpine.$data(dragDropOverlay).isVisible = true;
                } catch (error) {
                    console.warn('Error accessing Alpine data for dragDropOverlay:', error);
                }
            }
        }
    }, false);
    ...
    dragDropOverlay.addEventListener('drop', (e) => {
        ...
        if (window.Alpine && inputSection && inputSection._x_dataStack) {
            try {
                ...
                console.warn('Error accessing Alpine data for inputSection:', error);
            }
        }
    }, false);
});
...
window.handleFileUpload = function (event) {
    ...
    if (window.Alpine && inputSection && inputSection._x_dataStack) {
        try {
            ...
            console.warn('Error accessing Alpine data for file upload:', error);
        }
    }
}
    ...
    if (tabName === 'chats') {
        ...
        if (window.Alpine && chatsSection && chatsSection._x_dataStack) {
            try {
                ...
                console.warn('Error accessing Alpine data for chats section:', error);
            }
        }
        ...
        // Check if Alpine is available before using it
        if (window.Alpine && tasksSection && tasksSection._x_dataStack) {
            try {
                ...
                console.warn('Error accessing Alpine data for tasks section:', error);
            }
        }
    }
...
function initializeActiveTab() {
    ...
    // Wait for Alpine to be available before calling activateTab
    if (window.Alpine) {
        activateTab(activeTab);
    } else {
        // If Alpine is not ready, wait a bit and try again
        setTimeout(() => {
            if (window.Alpine) {
                activateTab(activeTab);
            } else {
                console.warn('Alpine not available for tab initialization');
            }
        }, 100);
    }
}
...
function openTaskDetail(taskId) {
    ...
    if (window.Alpine) {
        ...
        if (settingsButton) {
            ...
            // Check if modal has Alpine data before accessing it
            if (window.Alpine && modalEl && modalEl._x_dataStack) {
                try {
                    ...
                    setTimeout(() => {
                        ...
                        setTimeout(() => {
                            ...
                            // Check if scheduler component has Alpine data before accessing it
                            if (window.Alpine && schedulerComponent && schedulerComponent._x_dataStack) {
                                try {
                                    ...
                                    if (schedulerData.showTaskDetail) {
                                        schedulerData.showTaskDetail(taskId);
                                        console.log('Task detail view opened for task:', taskId);
                                    } else {
                                        console.error('showTaskDetail method not available');
                                    }
                                } catch (error) {
                                    console.error('Error accessing scheduler Alpine data:', error);
                                }
                            } else {
                                console.error('Scheduler component not initialized with Alpine data');
                            }
                        }, 50); // Give time for the scheduler tab to initialize
                    }, 25); // Give time for the modal to render
                } catch (error) {
                    console.error('Error accessing modal Alpine data:', error);
                }
            } else {
                console.error('Modal not initialized with Alpine data');
            }
        } else {
