...
document.addEventListener('alpine:init', () => {
    Alpine.data('tunnelSettings', () => ({
        ...
        async generateLink() {
            ...
            try {
                ...
                if (!hasAuth) {
                    const proceed = confirm(
                        "WARNING: No authentication is configured for your GPAF instance.\n\n" +
                        "Creating a public tunnel without authentication means anyone with the URL " +
                        "can access your GPAF instance.\n\n" +
            ...
            let provider = 'serveo'; // Default provider
            
            if (window.Alpine && modalEl && modalEl._x_dataStack) {
                try {
                    const modalAD = Alpine.$data(modalEl);
                    provider = modalAD.provider || 'serveo';
                } catch (error) {
                    console.warn('Error accessing Alpine data for modal provider:', error);
                }
            } else {
                // Fallback: try to get provider from the element itself
                const providerSelect = document.getElementById('tunnel-provider');
                if (providerSelect) {
                    provider = providerSelect.value || 'serveo';
                }
            }
