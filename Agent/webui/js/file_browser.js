  ...
  async openModal(path) {
    ...
    if (window.Alpine && modalEl && modalEl._x_dataStack) {
      try {
        ...
        console.warn('Error accessing Alpine data for file browser modal:', error);
        // Fallback: use proxy methods directly
        this.isOpen = true;
        this.isLoading = true;
        this.history = [];
        
        if (path) this.browser.currentPath = path;
        else if (!this.browser.currentPath)
          this.browser.currentPath = "$WORK_DIR";
