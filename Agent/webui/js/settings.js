    ...
    switchTab(tabName) {
        ...
        setTimeout(() => {
            ...
            if (tabName === 'scheduler') {
                ...
                if (schedulerElement && window.Alpine && schedulerElement._x_dataStack) {
                    try {
                        ...
                        console.warn('Error accessing Alpine data for scheduler element:', error);
                    }
                }
            }
            ...
            if (tabName === 'tunnel') {
                ...
                if (tunnelElement && window.Alpine && tunnelElement._x_dataStack) {
                    try {
                        ...
                        console.warn('Error accessing Alpine data for tunnel element:', error);
                    }
                }
            }
        }, 10);
    ...
    async openModal() {
        ...
        if (window.Alpine && modalEl && modalEl._x_dataStack) {
            try {
                ...
                console.error('Error accessing Alpine data for settings modal:', error);
                throw error;
            }
        } else {
            console.error('Alpine not available or modal not initialized');
            throw new Error('Settings modal not properly initialized');
        }
                    ...
                    if (modalAD.activeTab === 'scheduler') {
                        ...
                        if (schedulerElement && window.Alpine && schedulerElement._x_dataStack) {
                            try {
                                ...
                                console.warn('Error accessing Alpine data for scheduler element in openModal:', error);
                            }
                        }
                    }
                ...
                const checkSchedulerEditingState = () => {
                    if (window.Alpine && schedulerComponent && schedulerComponent._x_dataStack) {
                        try {
                            ...
                            console.warn('Error accessing Alpine data in checkSchedulerEditingState:', error);
                        }
                    }
                };
        ...
        if (buttonId === 'save') {
            ...
            if (window.Alpine && modalEl && modalEl._x_dataStack) {
                try {
                    ...
                    console.error('Error accessing Alpine data in handleButton:', error);
                    return;
                }
            } else {
                console.error('Alpine not available or modal not initialized in handleButton');
                return;
            }
            try {
    ...
    stopSchedulerPolling() {
        ...
        if (schedulerElement && window.Alpine && schedulerElement._x_dataStack) {
            try {
                ...
                console.warn('Error accessing Alpine data in stopSchedulerPolling:', error);
            }
        }
    },
            ...
            closeModal() {
                ...
                if (schedulerElement && window.Alpine && schedulerElement._x_dataStack) {
                    try {
                        ...
                        console.warn('Error accessing Alpine data in closeModal:', error);
                    }
                }
