...
# Global flag to track if handlers are registered
...
def register_api_handlers_once():
    """Register API handlers only once to avoid duplicates"""
    ...
    def register_api_handler(app, handler: type[ApiHandler]):
        ...
        # Check if endpoint already exists before adding
        endpoint = f"/{name}"
...
def run():
    ...
    # Register API handlers only once
    register_api_handlers_once()
...
# Global WSGI app for gunicorn
app = None

def create_app():
    """สร้าง WSGI app สำหรับ gunicorn"""
    global app
    if app is None and __name__ != "__main__":
        PrintStyle().print("Initializing framework for production...")
        ...
        # Create the dispatcher middleware app
        app = DispatcherMiddleware(
            webapp,
            {
        ...
        PrintStyle().print("✅ GPAF initialized successfully for production!")
    
    return app

# Create app instance for gunicorn (only when not running directly)
