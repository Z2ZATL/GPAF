...
The user or GPAF is at the top of the hierarchy, delegating tasks to subordinate agents, which can further delegate to other agents. Each agent can utilize tools and access the shared assets (prompts, memory, knowledge, extensions and instruments) to perform its tasks.
...
1. The user provides an instruction to GPAF
2. GPAF initializes VectorDB and access memory
3. GPAF analyzes the instruction and formulates a plan using `thoughts` argument, possibly involving the use of tools or the creation of sub-agents
4. If necessary, GPAF delegates sub-tasks to subordinate agents
...
7. GPAF provides the final response to the user
