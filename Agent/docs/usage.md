...
This guide provides detailed instructions on how to use GPAF effectively.

## Getting Started

1. **Access the Web UI**
   - Open your web browser
   - Navigate to `http://localhost:<PORT>` (where PORT is the port you configured)
   - The GPAF interface will load
...
GPAF provides various tools and functions:
...
- Long-term memory for learned information
...
You can customize GPAF's behavior by:
...
GPAF supports remote function calls for:
...
For more help, visit our [Discord community](https://discord.gg/Z2tun2N3) or [Skool](https://www.skool.com/gpaf).

![Utility Message with Solutions](res/memory-man.png)

## Basic Operations
GPAF provides several basic operations through its interface:

### Restart Framework
The Restart button allows you to quickly restart the GPAF framework without using the terminal:
...
### Action Buttons
Located beneath the chat input box, GPAF provides a set of action buttons for enhanced control and visibility:
...
### File Browser: Manage files in the GPAF environment
...
### File Attachments
GPAF supports direct file attachments in the chat interface for seamless file operations:
...
## Tool Usage
GPAF's power comes from its ability to use [tools](architecture.md#tools). Here's how to leverage them effectively:

- **Understand Tools:** GPAF includes default tools like knowledge (powered by SearXNG), webpage content, code execution, and communication. Understand the capabilities of these tools and how to invoke them.

## Example of Tools Usage: Web Search and Code Execution
Let's say you want GPAF to perform some financial analysis tasks. Here's a possible prompt:
...
> [!NOTE]
> The first run of `code_execution_tool` may take a while as it downloads and builds the GPAF Docker image. Subsequent runs will be faster.

This example demonstrates how to combine multiple tools to achieve an analysis task. By mastering prompt engineering and tool usage, you can unlock the full potential of GPAF to solve complex problems.

## Multi-Agent Cooperation
One of GPAF's unique features is multi-agent cooperation.
...
* **Hierarchy:** GPAF uses a [hierarchical structure](architecture.md#agent-hierarchy-and-communication), with superior agents delegating tasks to subordinates.  This allows for structured problem-solving and efficient resource allocation.

![](res/physics.png)
![](res/physics-2.png)

## Prompt Engineering
Effective prompt engineering is crucial for getting the most out of GPAF. Here are some tips and techniques:

* **Be Clear and Specific:** Clearly state your desired outcome.  The more specific you are, the better GPAF can understand and fulfill your request.  Avoid vague or ambiguous language.
...
## Voice Interface
GPAF provides both Text-to-Speech (TTS) and Speech-to-Text (STT) capabilities for natural voice interaction:
...
GPAF supports KaTeX for beautiful mathematical typesetting:
...
### File Browser
GPAF provides a powerful file browser interface for managing your workspace:
...
> [!NOTE]
> The files browser allows the user to go in the GPAF root folder if you click the `Up` button, but the working directory of Agents will always be `/work_dir`
> 
- **File Operations**:
...
> [!TIP]
> The File Browser integrates seamlessly with GPAF's capabilities. You can reference files directly in your conversations, and the agent can help you manage, modify, and organize your files.