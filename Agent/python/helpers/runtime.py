...
async def call_development_function(func: Union[Callable[..., T], Callable[..., Awaitable[T]]], *args, **kwargs) -> T:
    # ตรวจสอบ DISABLE_RFC environment variable
    if os.getenv("DISABLE_RFC", "false").lower() == "true":
        # RFC ถูก disable ให้เรียกฟังก์ชันโดยตรง
        ...
        try:
            ...
            # ถ้าการเชื่อมต่อ RFC ล้มเหลว ให้เรียกฟังก์ชันโดยตรง
            if inspect.iscoroutinefunction(func):
                return await func(*args, **kwargs)
            else:
                return func(*args, **kwargs) # type: ignore
